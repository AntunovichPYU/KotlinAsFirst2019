...
class Tests {
    ...
    fun lineBySegment() {
        ...
        assertApproxEquals(Line(Point(3.0, 1.0), 3 * PI / 4), lineBySegment(Segment(Point(1.0, 3.0), Point(3.0, 1.0))))
    }
    ...
    fun bisectorByPoints() {
        ...
        assertApproxEquals(Line(Point(2.0, 2.0), PI / 4), bisectorByPoints(Point(1.0, 3.0), Point(3.0, 1.0)))
    }
    ...
    fun circleByThreePoints() {
        ...
        val res = circleByThreePoints(
            Point(0.18598444607659936, -632.0),
            Point(-632.0, 0.0),
            Point(0.8267722034521447, -2.220446049250313e-16)
        )
        assertTrue(res.center.distance(Point(-315.5866138982739, -315.67951183606215)) < 1e-5)
        assertEquals(446.9574757148766, res.radius, 1e-5)
    }
